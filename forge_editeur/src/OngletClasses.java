...
class OngletClasses extends JPanel
{
	private static final long serialVersionUID = 1L;
	JTabbedPane TypesClasses;
	ClassesJoueur cj;
	ClassesMonstre cm;
	
	public OngletClasses(FenetreSimple p)
	{
	  setLayout(new BorderLayout());
	  TypesClasses = new JTabbedPane();
	  cj=new ClassesJoueur(p);
	  TypesClasses.addTab("Joueurs",cj);
	  cm=new ClassesMonstre(p);
	  TypesClasses.addTab("Monstres",cm);
	  add(TypesClasses);
	}
	...
	class ClassesMonstre extends JPanel
	{
		private static final long serialVersionUID = 1L;
		FenetreSimple parent;
		DefaultListModel Classemodel; 
		public JList ClasseList;
		JScrollPane scrollpaneclasslist;
		JPanel paneuneclasse;
		JTextField Ed_Nom,Ed_FormuleAtt,Ed_FormuleEsq,Ed_FormuleDeg,Ed_FormuleDef;
		JButton Bt_FormuleAtt,Bt_FormuleEsq,Bt_FormuleDeg,Bt_FormuleDef;
		public ClassesMonstre(FenetreSimple p)
		{
		    setLayout(null);
			parent=p;
			...
			ClasseList.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					if (ClasseList.getSelectedIndex()>=0)
					{
						...
						Ed_Nom.setText(parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).Name);
						Ed_FormuleAtt.setText(parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleAttaque);
						Ed_FormuleEsq.setText(parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleEsquive);
						Ed_FormuleDeg.setText(parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleDegat);
						Ed_FormuleDef.setText(parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleDefense);
					}
				}
			});
			...
			Bt_AjouteClasse.addActionListener(new ActionListener(){
				...
				{
					String NomClasse = JOptionPane.showInputDialog(null, "Entrez le nom de la nouvelle classe", 
							"", 1);
					if (NomClasse!=null)
					{
						if (NomClasse.compareTo("")!=0)
						{
							...
							parent.general.getClassesMonstre().add(parent.general.new ClasseMonstre(NomClasse));
				...
				public void actionPerformed(ActionEvent e)
				{
					if (ClasseList.getSelectedIndex()>=0)
					{
				    	if (JOptionPane.showConfirmDialog(null,
				                "Etes vous sûr de vouloir effacer cette classe?",
				                "Effacer",
				                ...
				                JOptionPane.WARNING_MESSAGE)==JOptionPane.YES_OPTION)
				    	{
							...
							{
								if (Classemodel.size()>1)
								{
									if (parent.general.getMonstreByIndex(i).ClasseMonstre>=ClasseList.getSelectedIndex())
										parent.general.getMonstreByIndex(i).ClasseMonstre--;
								}
								else
									parent.general.getMonstreByIndex(i).ClasseMonstre=0;
							}
								
							parent.general.getClassesMonstre().remove(ClasseList.getSelectedIndex());
			...
			Ed_Nom.setBounds(new Rectangle(170,0,380,20));
			Ed_Nom.addKeyListener(new KeyListener() {
					...
					public void keyReleased(KeyEvent e) {
			    		  parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).Name=Ed_Nom.getText();
			...
			ActionListener stwizard=new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
		    	      ...
		    	      for(int i=0;i<parent.general.getStatsBase().size();i++)
		    	    	  values[i]="%"+parent.general.getStatsBase().get(i)+"%";
		    	      compte=parent.general.getStatsBase().size();
		    	      values[compte]="%Vie%"; values[compte+1]="%VieMax%";
		    	      values[compte+2]="%CurrentMag%"; values[compte+3]="%MagMax%"; values[compte+4]="%Gold%";
		    	      values[compte+5]="%Lvl%"; values[compte+6]="%LvlPoint%"; values[compte+7]="%CurrentXP%";
		    	      values[compte+8]="%NextXP%";  values[compte+9]="%rand(100)%"; values[compte+10]="%max(valeur1,valeur2)%";
		    	      values[compte+11]="%min(valeur1,valeur2)%"; 
		    	      values[compte+12]="%Arme.Prix%"; values[compte+13]="%Arme.Attaque%";
		    	      values[compte+14]="%Arme.Defense%";  values[compte+15]="%Arme.Vie%";
		    	      values[compte+16]="%Arme.Magie%";
		    	      values[compte+17]="%Armure.Prix%"; values[compte+18]="%Armure.Attaque%";
		    	      values[compte+19]="%Armure.Defense%";  values[compte+20]="%Armure.Vie%";
		    	      values[compte+21]="%Armure.Magie%";
		    	      values[compte+22]="%Bouclier.Prix%"; values[compte+23]="%Bouclier.Attaque%";
		    	      values[compte+24]="%Bouclier.Defense%";  values[compte+25]="%Bouclier.Vie%";
		    	      values[compte+26]="%Bouclier.Magie%";
		    	      values[compte+27]="%Casque.Prix%"; values[compte+28]="%Casque.Attaque%";
		    	      values[compte+29]="%Casque.Defense%";  values[compte+30]="%Casque.Vie%";
		    	      values[compte+31]="%Casque.Magie%";
		    	      values[compte+32]="Variable[nomvar]";
		    	      String [] cible=new String[] {"%Monstre.Attaque%","%Monstre.Esquive%","%Monstre.Vie%","%Monstre.Lvl%","%Monstre.VieMax%","%Monstre.Degat%","%Monstre.Defense%","%Monstre.XPMin%",
								 "%Monstre.XPMax%","%Monstre.GoldMin%","%Monstre.GoldMax%"};
		...
		{
			parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleAttaque=Ed_FormuleAtt.getText();
			parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleEsquive=Ed_FormuleEsq.getText();
			parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleDefense=Ed_FormuleDef.getText();
			parent.general.getClassesMonstre().get(ClasseList.getSelectedIndex()).FormuleDegat=Ed_FormuleDeg.getText();
		}
		...
		public void StatsBaseChange()
		{
			if (parent.general!=null)
			{
		    	...
		    	for(int i=0;i<parent.general.getClassesMonstre().size();i++)
		    		Classemodel.add(i, parent.general.getClassesMonstre().get(i).Name);
			}
		}
	}
	
	class ClassesJoueur extends JPanel
	{
		private static final long serialVersionUID = 1L;
		FenetreSimple parent;
		DefaultListModel Statmodel,Classemodel; 
		public JList StatBase,ClasseList;
		JScrollPane scrollpaneclasslist;
		JTable StatsMinGrid,StatsMaxGrid;
		JPanel paneuneclasse;
		JTextField Ed_Nom,Ed_LvlUpPoint,Ed_FormuleAtt,Ed_FormuleDeg,Ed_FormuleDef,Ed_FormuleEsq,Ed_FormuleXP,Ed_FormuleGold;
		JTextField Ed_FormuleVieMax,Ed_FormuleMagMax;
		JButton Bt_Nom,Bt_FormuleAtt,Bt_FormuleDeg,Bt_FormuleDef,Bt_FormuleXP,Bt_FormuleGold;
		JButton Bt_FormuleVieMax,Bt_FormuleMagMax,Bt_FormuleEsq;
		JTextField Ed_SonAttaque,Ed_SonBlesse,Ed_SonMagie,Ed_LvlMax;
			...
			ActionListener ChooseSound=new ActionListener(){
				public void actionPerformed(ActionEvent e)
				{
				     ...
				     if(retour == JFileChooser.APPROVE_OPTION) 
				     {
				    	 ...
				    	 if (e.getSource().equals(Bt_ChooseSonAttaque))
				    		 Edit=Ed_SonAttaque;
				    	 else if (e.getSource().equals(Bt_ChooseSonBlesse))
				    		 Edit=Ed_SonBlesse;
				    	 else if (e.getSource().equals(Bt_ChooseSonMagie))
				    		 Edit=Ed_SonMagie;
				    	 if (Edit!=null)
		...
		private void SaveClasses()
		{
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleAttaque=Ed_FormuleAtt.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleEsquive=Ed_FormuleEsq.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleDegat=Ed_FormuleDeg.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleDefense=Ed_FormuleDef.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleXP=Ed_FormuleXP.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleGold=Ed_FormuleGold.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleVieMax=Ed_FormuleVieMax.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).FormuleMagMax=Ed_FormuleMagMax.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).SoundAttaque=Ed_SonAttaque.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).SoundWound=Ed_SonBlesse.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).SoundConcentration=Ed_SonMagie.getText();
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).LvlupPoint=Integer.parseInt(Ed_LvlUpPoint.getText());
			parent.general.getClassesJoueur().get(ClasseList.getSelectedIndex()).LvlMax=Integer.parseInt(Ed_LvlMax.getText());
		}
		public void StatsBaseChange()
		{
			if (parent.general!=null)
			{
		    	...
		    	for(int i=0;i<parent.general.getClassesJoueur().size();i++)
		    		Classemodel.add(i, parent.general.getClassesJoueur().get(i).Name);
			}
		}
	   public class StdGridModel extends AbstractTableModel {
			...
			public StdGridModel(String colname,ArrayList<Integer> stat)
			{
			 ...
			 public String getColumnName(int column) {
				    switch(column)
				    {
				    	case 0 : return ColName; 
				    	case 1 : return "Valeur";
				    	default : return "unavailable";
				    }
			}
			public int getColumnCount() { return 2; }
		    public int getRowCount() { if (parent.general!=null) return parent.general.getStatsBase().size(); else return 0;}
		    public String getValueAt(int row, int col) {
		    	switch(col)
		    	{
		    		...
		    		case 1 :
		    			if (Stats!=null)
		    				return Integer.toString(Stats.get(row));
		    			else
		    				return "0";
		    		default :
		    			return "unavailable";
