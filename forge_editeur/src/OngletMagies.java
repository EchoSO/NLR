...
class OngletMagies extends JPanel
{
	private static final long serialVersionUID = 1L;
	FenetreSimple parent;
	DefaultListModel Magiemodel; 
	public JList MagieList;
	JScrollPane scrollpanemaglist;
	JPanel paneunemagie;
	JTextField Ed_Nom,Ed_Explication,Ed_Chipset;
	JTextField Ed_X,Ed_Y,Ed_W,Ed_H,Ed_Tran;
	JTextField Ed_SonMagie,Ed_FormuleZone,Ed_FormuleDuree, Ed_FormuleTouche;
	JTextField Ed_FormuleEffet,Ed_MPNeeded,Ed_LvlMin,Ed_TempsIncantation,Ed_DureeAnim;
	JComboBox CB_Classe;
	JRadioButton RB_SortCibleUnique,RB_Bouledefeu,RB_EffetZone,RB_Teleportation,RB_Resurrection;
	JRadioButton RB_SautAttaque,RB_AttaqueSournoise,RB_PosDynamique,RB_PosAuDessus;
	JRadioButton RB_CibleMonstre,RB_CibleJoueur,RB_AutourJoueur;
	JButton Bt_ChooseSonMagie,Bt_FormuleZone,Bt_FormuleDuree, Bt_FormuleTouche,Bt_FormuleEffet;
	...
	public OngletMagies(FenetreSimple p)
	{
			...
			public void mousePressed(MouseEvent e) {
				if (MagieList.getSelectedIndex()>=0)
				{
					...
					for(int i=0;i<parent.general.getClassesJoueur().size();i++)
						CB_Classe.addItem(parent.general.getClassesJoueur().get(i).Name);
				    ...
				    {
				    	case 0 : RB_SortCibleUnique.setSelected(true); break;
				    	case 1 : RB_Bouledefeu.setSelected(true); break;
				    	case 2 : RB_EffetZone.setSelected(true); break;
				    	case 3 : RB_Teleportation.setSelected(true); break;
				    	case 4 : RB_Resurrection.setSelected(true); break;
				    	case 5 : RB_SautAttaque.setSelected(true); break;
				    	case 6 : RB_AttaqueSournoise.setSelected(true); break;
				    }
				    switch(parent.general.getMagieByIndex(MagieList.getSelectedIndex()).OnMonster)
				    {
				    	case 0 : RB_CibleMonstre.setSelected(true); break;
				    	case 1 : RB_CibleJoueur.setSelected(true); break;
				    	case 2 : RB_AutourJoueur.setSelected(true); break;
				    }
				    switch(parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Z)
				    {
				    	case 0 : RB_PosDynamique.setSelected(true); break;
				    	case 1 : RB_PosAuDessus.setSelected(true); break;
				    }
				    LoadImage();
				    AllowSave=true;
				}
		...
		Bt_AjouteMagie.addActionListener(new ActionListener(){
			...
			{
				String NomMagie = JOptionPane.showInputDialog(null, "Entrez le nom de la nouvelle magie", 
						"", 1);
				if (NomMagie!=null)
				{
			...
			public void actionPerformed(ActionEvent e)
			{
				if (MagieList.getSelectedIndex()>=0)
				{
			    	if (JOptionPane.showConfirmDialog(null,
			                "Etes vous sûr de vouloir effacer cette magie?",
			                "Effacer",
			                ...
			                JOptionPane.WARNING_MESSAGE)==JOptionPane.YES_OPTION)
			    	{
			    		...
			    		for (int i=0;i<parent.general.getMonstres().size();i++)
			    		{
			    			...
			    			{
			    				if (parent.general.getMonstreByIndex(i).Spell.get(j)>MagieList.getSelectedIndex())
			    					parent.general.getMonstreByIndex(i).ObjectWin.set(j, (short) (parent.general.getMonstreByIndex(i).ObjectWin.get(j)-1));
			    			}
			    		}
		...
		KeyListener keyListener = new KeyListener() {
		      ...
		      public void keyReleased(KeyEvent keyEvent) {
		    	  if ((keyEvent.getSource().equals(Ed_X))||(keyEvent.getSource().equals(Ed_Y))
		    	     ||(keyEvent.getSource().equals(Ed_W))||(keyEvent.getSource().equals(Ed_H)))
		    	     RefreshImage();
		    	  SaveMagie();
		      }
		};
		...
		ActionListener ChooseSound=new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
			     ...
			     if(retour == JFileChooser.APPROVE_OPTION) 
			     {
			    	 ...
			    	 if (e.getSource().equals(Bt_ChooseSonMagie))
			    		 Edit=Ed_SonMagie;
			    	 if (Edit!=null)
			    		 Edit.setText("Sound\\"+choix.getSelectedFile().getName());
			    	 SaveMagie();
			     }
			}
		};
		ActionListener stwizard=new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
	    		  ...
	    		  if (e.getSource()==Bt_FormuleZone) { s=Ed_FormuleZone.getText(); commentaire="Zone d'effet du sort. Le sort sera aggrandi X fois le chiffre résultant de la formule"; }  
	    		  if (e.getSource()==Bt_FormuleDuree) { s=Ed_FormuleDuree.getText(); commentaire="Durée du sort. Le sort durera X fois le chiffre résultant de la formule"; }
	    		  if (e.getSource()==Bt_FormuleTouche) { s=Ed_FormuleTouche.getText(); commentaire="Formule qui détermine si le sort touche sa cible ou non. Un chiffre > 0 touche, sinon le sort rate sa cible"; } 
	    		  ...
	    		  { 
	    			  ...
	    			  for(int j=0;j<parent.general.getMagies().size();j++)
	    				  combospecial[j+1]=parent.general.getMagieByIndex(j).Name;
	    		  } 
	    		  int compte;
	    	      ...
	    	      for(int i=0;i<parent.general.getStatsBase().size();i++)
	    	    	  values[i]="%Wizard."+parent.general.getStatsBase().get(i)+"%";
	    	      compte=parent.general.getStatsBase().size();
	    	      values[compte]="%Wizard.Attaque%"; values[compte+1]="%Wizard.Esquive%";
	    	      values[compte+2]="%Wizard.Vie%"; values[compte+3]="%Wizard.VieMax%";
	    	      values[compte+4]="%Wizard.CurrentMag%"; values[compte+5]="%Wizard.MagMax%"; values[compte+6]="%Wizard.Gold%";
	    	      values[compte+7]="%Wizard.Lvl%"; values[compte+8]="%Wizard.LvlPoint%"; values[compte+9]="%Wizard.CurrentXP%";
	    	      values[compte+10]="%Wizard.NextXP%";  values[compte+11]="%rand(100)%"; values[compte+12]="%max(valeur1,valeur2)%";
	...
	private void SaveMagie()
	{		
		if (AllowSave)
		{
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Name=Ed_Nom.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Explication=Ed_Explication.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Chipset=Ed_Chipset.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).X=Integer.parseInt(Ed_X.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Y=Integer.parseInt(Ed_Y.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).W=Integer.parseInt(Ed_W.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).H=Integer.parseInt(Ed_H.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Tran=Integer.parseInt(Ed_Tran.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).SoundMagie=Ed_SonMagie.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).FormuleZone=Ed_FormuleZone.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).FormuleDuree=Ed_FormuleDuree.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).FormuleTouche=Ed_FormuleTouche.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).FormuleEffet=Ed_FormuleEffet.getText();
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MPNeeded=(short) Integer.parseInt(Ed_MPNeeded.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).LvlMin=(short) Integer.parseInt(Ed_LvlMin.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).TempsIncantation=(short) Integer.parseInt(Ed_TempsIncantation.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).DureeAnim=(short) Integer.parseInt(Ed_DureeAnim.getText());
			parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Classe=(short) CB_Classe.getSelectedIndex();
			if (RB_SortCibleUnique.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=0; 
			if (RB_Bouledefeu.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=1; 
			if (RB_EffetZone.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=2; 
			if (RB_Teleportation.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=3; 
			if (RB_Resurrection.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=4; 
			if (RB_SautAttaque.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=5; 
			if (RB_AttaqueSournoise.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).MagieType=6; 
			if (RB_CibleMonstre.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).OnMonster=0; 
			if (RB_CibleJoueur.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).OnMonster=1; 
			if (RB_AutourJoueur.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).OnMonster=2; 
			if (RB_PosDynamique.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Z=0; 
			if (RB_PosAuDessus.isSelected()) parent.general.getMagieByIndex(MagieList.getSelectedIndex()).Z=1; 
		}
	}
	...
	private void RefreshImage()
	{
		if (imchipset!=null)
		{
			ImageFilter cif2 =new CropImageFilter(Integer.parseInt(Ed_X.getText()),
								Integer.parseInt(Ed_Y.getText()),
	...
	public void StatsBaseChange()
	{
		if (parent.general!=null)
		{
			paneunemagie.setVisible(false);
		    Magiemodel.clear();
	    	for(int i=0;i<parent.general.getMagies().size();i++)
	    		Magiemodel.add(i, parent.general.getMagieByIndex(i).Name);
		}
	}
}
