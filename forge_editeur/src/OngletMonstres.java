...
class OngletMonstres extends JPanel
{
	...
	public OngletMonstres(FenetreSimple p)
	{
				...
				if (MonstreList.getSelectedIndex()>=0)
				{
					...
					for(int i=0;i<GainObj.getColumnCount();i++)
					{
					  ...
					  if (i==1)
					  {
						  ...
						  for(int j=0;j<parent.general.getObjets().size();j++)
							  values[j+1]=parent.general.getObjetByIndex(j).Name;
					...
					for(int i=0;i<Sort.getColumnCount();i++)
					{
					  ...
					  if (i==1)
					  {
						  ...
						  for(int j=0;j<parent.general.getMagies().size();j++)
							  values[j+1]=parent.general.getMagieByIndex(j).Name;
		...
		Bt_AjouteMonstre.addActionListener(new ActionListener(){
			...
			{
				String NomMonstre = JOptionPane.showInputDialog(null, "Entrez le nom du nouveau monstre", 
						"", 1);
				if (NomMonstre!=null)
				{
			...
			public void actionPerformed(ActionEvent e)
			{
				if (MonstreList.getSelectedIndex()>=0)
				{
			    	if (JOptionPane.showConfirmDialog(null,
			                "Etes vous sûr de vouloir effacer ce monstre?",
			                "Effacer",
		...
		KeyListener keyListener = new KeyListener() {
		      ...
		      public void keyReleased(KeyEvent keyEvent) {
		    	  if ((keyEvent.getSource().equals(Ed_W))||(keyEvent.getSource().equals(Ed_H)))
		    	     RefreshImage();
		    	  SaveMonstre();
		      }
		};
	...
	private void RefreshImage()
	{
		if (imchipset!=null)
		{
			ImageFilter cif2 =new CropImageFilter(Integer.parseInt(Ed_W.getText()),Integer.parseInt(Ed_H.getText())*2,
	...
	public void StatsBaseChange()
	{
		if (parent.general!=null)
		{
			...
			if (parent.general.getClassesMonstre().size()>0)
			{
				...
				CB_ClasseMonstre.addItem("Aucune classe existante");
		    Monstremodel.clear();
	    	for(int i=0;i<parent.general.getMonstres().size();i++)
	    		Monstremodel.add(i, parent.general.getMonstreByIndex(i).Name);
    ...
    public class ObjGridModel extends AbstractTableModel {
	    ...
	    public int getRowCount() { if (parent.general!=null) return parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).ObjectWin.size(); else return 0;}
	    public String getValueAt(int row, int col) {
	    	switch(col)
	    	{
	    		...
	    		case 1 :
	    			if (parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).ObjectWin.get(row)==0)
	    				return "Pas d'objet";
	    			else
	    				return parent.general.getObjetByIndex(parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).ObjectWin.get(row)-1).Name;
	    		default :
	    			return "unavailable";
	    	}
	    }
	    ...
	    public void setValueAt(Object value, int row, int col) {
	    	switch(col)
	    	{
	    	  case 0 : parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).PercentWin.set(row,(short) Integer.parseInt((String) value)); break;
	    	  case 1 :
	    		  if (((String) value).compareTo("Pas d'objet")==0)
	    			  parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).ObjectWin.set(row,(short) 0);
	    		  else 
	    		  for (int i=0;i<parent.general.getObjets().size();i++)
	    		  {
	    			  if (parent.general.getObjetByIndex(i).Name.compareTo((String) value)==0)
	    				  	parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).ObjectWin.set(row,(short) (i+1));
	    		  }
	    		  break;
	    	}
    ...
    public class MagGridModel extends AbstractTableModel {
		...
		public int getColumnCount() { return 2; }
	    public int getRowCount() { if (parent.general!=null) return parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).Spell.size(); else return 0;}
	    public String getValueAt(int row, int col) {
	    	switch(col)
	    	{
	    		case 0 : 
	    		...
	    		case 1 :
	    			if (parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).Spell.get(row)==0)
	    				return "Pas de magie";
	    			else
	    				return parent.general.getMagieByIndex(parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).Spell.get(row)-1).Name;
	    		default :
	    			return "unavailable";
	    	}
	    }
	    ...
	    public void setValueAt(Object value, int row, int col) {
	    	switch(col)
	    	{
	    	  case 0 : parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).LuckSpell.set(row,(short) Integer.parseInt((String) value)); break;
	    	  case 1 :
	    		  if (((String) value).compareTo("Pas de magie")==0)
	    			  parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).Spell.set(row,(short) 0);
	    		  else 
	    		  for (int i=0;i<parent.general.getMagies().size();i++)
	    		  {
	    			  if (parent.general.getMagieByIndex(i).Name.compareTo((String) value)==0)
	    				  	parent.general.getMonstreByIndex(MonstreList.getSelectedIndex()).Spell.set(row,(short) (i+1));
	    		  }
	    		  break;
	    	}
            fireTableCellUpdated(row, col);
