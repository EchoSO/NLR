...
class OngletObjets extends JPanel
{
	...
	public OngletObjets(FenetreSimple p)
	{
			...
			public void mousePressed(MouseEvent e) {
				if (ObjetList.getSelectedIndex()>=0)
				{
					...
					for(int i=0;i<parent.general.getMagies().size();i++)
						CB_MagieAssoc.addItem(parent.general.getMagieByIndex(i).Name);
				    ...
				    {
				    	case 0 : RB_Utilise.setSelected(true); break;
				    	case 1 : RB_Armeunemain.setSelected(true); break;
				    	case 2 : RB_Armedeuxmain.setSelected(true); break;
				    	case 3 : RB_Casque.setSelected(true); break;
				    	case 4 : RB_Armure.setSelected(true); break;
				    	case 5 : RB_Bouclier.setSelected(true); break;
				    	case 6 : RB_Objetquete.setSelected(true); break;
				    	case 7 : RB_Livremagie.setSelected(true); break;
				    }
				    LoadImage();
				    AllowSave=true;
				}
		...
		Bt_AjouteObjet.addActionListener(new ActionListener(){
			...
			{
				String NomObjet = JOptionPane.showInputDialog(null, "Entrez le nom du nouvel objet", 
						"", 1);
				if (NomObjet!=null)
				{
			...
			public void actionPerformed(ActionEvent e)
			{
				if (ObjetList.getSelectedIndex()>=0)
				{
			    	if (JOptionPane.showConfirmDialog(null,
			                "Etes vous sûr de vouloir effacer cet objet?",
			                "Effacer",
			                ...
			                JOptionPane.WARNING_MESSAGE)==JOptionPane.YES_OPTION)
			    	{
			    		...
			    		for (int i=0;i<parent.general.getMonstres().size();i++)
			    		{
			    			...
			    			{
			    				if (parent.general.getMonstreByIndex(i).ObjectWin.get(j)>ObjetList.getSelectedIndex())
	...
	public void StatsBaseChange()
	{
		if (parent.general!=null)
		{
	    	...
	    	for(int i=0;i<parent.general.getObjets().size();i++)
	    		Objetmodel.add(i, parent.general.getObjetByIndex(i).Name);
		}
	}
